...
cli.responders.help = () => {
  const commands = {
    ...
    "list checks --up --down":
      'Show the list of all active checks in the system, including their state. The "--up" and the "--down" flags are both optional',
    ...
    if (!err && checkIds && checkIds.length > 0) {
      ...
      checkIds.forEach(checkId => {
        _data.read("checks", checkId, (err, checkData) => {
          ...
          // If the user has specified the state, include the current check accordingly
          if (
